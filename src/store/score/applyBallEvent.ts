...
export const applyBallEvent = (state: ScoreType, event: BallEvent) => {
    ...
    innings.bowling?.forEach(b => {
        if (b.id === event.currentOver?.bowlerId) {
            b.r += event.bowlerRuns;
            if (event.isLegalBall) b.b += 1;
            if (event.batsmanRuns === 4) b['4s'] = (b['4s'] || 0) + 1;
            if (event.batsmanRuns === 6) b['6s'] = (b['6s'] || 0) + 1;
        }
    })
